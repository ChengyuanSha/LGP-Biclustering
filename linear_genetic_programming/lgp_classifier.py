...
class LGPClassifier(BaseEstimator, ClassifierMixin):
    ...
    def biclustering_evolution(self, df, max_bi_iteration):
        """
        ...
        # self.numberOfInput = X_disease.shape[1] # disease dimension
        # initializations
        self.__generateRegister()
        # generate random population
        p1 = Population()
        p1.generatePopulation(self.numberOfOperation, self.numberOfVariable, self.numberOfInput, self.numberOfConstant,
                             self.pConst, self.max_prog_ini_length, self.min_prog_ini_length, self.populationSize)

        p2 = Population()
        p2.generatePopulation(self.numberOfOperation, self.numberOfVariable, self.numberOfInput, self.numberOfConstant,
                             self.pConst, self.max_prog_ini_length, self.min_prog_ini_length, self.populationSize)
        ...
        pop2_y = y_disease[kmeans.labels_ == 1]


        row_labels = []
        iter = 0
        while iter < max_bi_iteration:
            print("iteration:", iter)

            pop1_balanced = np.append(pop1, X_ctrl[0:pop1.shape[0], :], axis=0)
            pop1_y_balanced = np.append(pop1_y, y_ctrl[0:pop1.shape[0]], axis=0)
            pop2_balanced = np.append(pop2, X_ctrl[pop1.shape[0]:(pop1.shape[0] + pop2.shape[0]), :], axis=0)
            pop2_y_balanced = np.append(pop2_y, y_ctrl[pop1.shape[0]:(pop1.shape[0] + pop2.shape[0])], axis=0)

            # convert to python lists
            # pop1_y_balanced = pop1_y_balanced.tolist()
            # pop2_y_balanced = pop2_balanced.tolist()

            e1 = Evolve(self.tournamentSize, self.maxGeneration, p1)  # tournamentSize, maxGeneration, population
            _ = e1.evolveGeneration(self.pRegmut, self.pMicro, self.pMacro, self.pConst, self.pCrossover,
                                          self.numberOfVariable, self.numberOfInput, self.numberOfOperation,
                                          self.numberOfConstant, self.register_, self.pInsert, self.maxProgLength,
                                          self.minProgLength, pop1_balanced, pop1_y_balanced, self.fitnessThreshold, self.showGenerationStat,
                                          self.isRandomSampling)
            pop_result1 = e1.p.population


            e2 = Evolve(self.tournamentSize, self.maxGeneration, p2)  # tournamentSize, maxGeneration, population
            _ = e2.evolveGeneration(self.pRegmut, self.pMicro, self.pMacro, self.pConst, self.pCrossover,
                                          self.numberOfVariable, self.numberOfInput, self.numberOfOperation,
                                          self.numberOfConstant, self.register_, self.pInsert, self.maxProgLength,
                                          self.minProgLength, pop2_balanced, pop2_y_balanced, self.fitnessThreshold, self.showGenerationStat,
                                          self.isRandomSampling)
            pop_result2 = e2.p.population

            biclustering_matrix = np.zeros( (len(pop_result1) + len(pop_result2), X_disease.shape[0]) )

            population_combined = pop_result1 + pop_result2

            for row, program in enumerate(population_combined):
                for col, sample in enumerate(X_disease):
                    biclustering_matrix[row, col] = program.predictProbaSigmoid(self.numberOfVariable, self.register_, sample, self.classes_)

            #print(biclustering_matrix)

            model = SpectralBiclustering(n_clusters=2, method='log', random_state=self.randomState)
            model.fit(biclustering_matrix)


            p1.population = [population_combined[i] for i, _ in enumerate(model.row_labels_) if model.row_labels_[i] == 0]
            p2.population =  [population_combined[i] for i, _ in enumerate(model.row_labels_) if model.row_labels_[i] == 1]

            print("row labels:", model.row_labels_)

            row_labels.append(model.row_labels_)

            iter += 1

        i = 0
        percentage = []
        while i + 1 < len(row_labels):
            percentage.append(np.mean(row_labels[i] == row_labels[i+1]))
            i += 1
        print("Percentage", percentage)
        sns.set()
        sns.scatterplot(range(len(percentage)), percentage)
        plt.show()
